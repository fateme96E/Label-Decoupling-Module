...
class SCP_Experiment():
    ...
    def prepare(self):
        ...
        print("x_train:{}, x_eval:{}, x_test:{}, class_num:{}".format(self.X_train.shape, self.X_val.shape, self.X_test.shape, self.n_classes))
    ...
    def perform(self):
        ...
        for model_description in self.models:
            ...
            if modeltype == "fastai_model":
                from models.fastai_model_decoupled import fastai_model
    ...
    def evaluate(self, output_name, n_bootstraping_samples=100, n_jobs=20, bootstrap_eval=False, dumped_bootstraps=True):
        ...
        for m in sorted(os.listdir(self.outputfolder+self.experiment_name+'/models')):
            ...
            # dump results
            te_df_result.to_csv(rpath+output_name)
